...
class AmzMetaData {
    ...
    public static function encodeObject($data) {
        $encodeData = utf8_encode(json_encode($data, JSON_UNESCAPED_SLASHES));
        $encodeCrc32 = dechex(crc32($encodeData));
        return strtoupper($encodeCrc32) . "#" . $encodeData;
    }
    ...
    public static function decryptMetaData1($ciphertext) {
        $key = AmzMetaData::long2str(AmzMetaData::$key, false);
        return AmzMetaData::decrypt(base64_decode($ciphertext), $key);
    }
}